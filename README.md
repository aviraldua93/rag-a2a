...
flowchart TD
    subgraph Ingestion["📄 Ingestion"]
        A[Documents<br/>MD · TS · JSON · TXT] --> B[Chunker<br/>sliding-window · semantic · recursive]
        B --> C[Embedder<br/>OpenAI / Mock]
        C --> D[Vector Store<br/>Qdrant / In-Memory]
    end

    subgraph Retrieval["🔍 Retrieval"]
        E[Vector Search] --> G[Reciprocal Rank Fusion]
        F[BM25 Keyword] --> G
        G --> H[Reranker]
    end

    subgraph Generation["🤖 Generation"]
        I[Context + Query] --> J[LLM<br/>GPT-4o-mini / Mock]
        J --> K[Streaming SSE Response]
    end

    subgraph Protocol["🌐 A2A Protocol"]
        L[Agent Card] ~~~ M[JSON-RPC 2.0]
        M ~~~ N[message/send · tasks/*]
    end
    ...
    style Ingestion fill:#1e3a5f,stroke:#4a9eed,color:#e0e0e0
    style Retrieval fill:#3a1e5f,stroke:#9b59b6,color:#e0e0e0
    style Generation fill:#1e5f3a,stroke:#2ecc71,color:#e0e0e0
    style Protocol fill:#5f3a1e,stroke:#e67e22,color:#e0e0e0
```
