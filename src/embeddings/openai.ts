...
/** Default max concurrent embedding API calls. */
 ...
 * transient / rate-limit errors, and a semaphore to cap concurrency.
 *
 * @example
 * ```ts
...
export class OpenAIEmbedder implements EmbeddingProvider {
  ...
  constructor(apiKey: string, model?: string) {
    ...
    const concurrency = parseInt(
      process.env.EMBEDDING_CONCURRENCY ?? String(DEFAULT_EMBEDDING_CONCURRENCY),
      10,
    );
    this.semaphore = new Semaphore(
      Number.isFinite(concurrency) && concurrency >= 1 ? concurrency : DEFAULT_EMBEDDING_CONCURRENCY,
    );
  }
  ...
  /** Embed a batch of texts in a single API call with retry logic, rate-limited by semaphore. */
